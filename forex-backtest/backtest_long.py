...
# Define Strategy
class MomentumLongOnlyStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        else:  # Manage Open Trade (Long Positions Only)
            if self.data.close[0] >= self.entry_price * (1 + self.params.take_profit) or \
...
# Backtesting Engine
cerebro = bt.Cerebro()
cerebro.addstrategy(MomentumLongOnlyStrategy, stop_loss=0.005, take_profit=0.01)
